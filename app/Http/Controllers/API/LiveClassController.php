...
class LiveClassController extends Controller
{
    ...
    {
        $classes = LiveClass::select('id','class_id', 'subject_id', 'chapter_id', 'title', 'description', 'image')->whereActive(1)->get();
        return $this->apiResponse(200, 'success', 'Live Classes lists', ['classes' => $classes]);
    }

    public function classSessions($liveClassId)
    {
        $sessions = LiveSession::select('id','live_class_id', 'date', 'timing', 'link', 'description')->where('live_class_id',$liveClassId)->whereActive(1)->get();
